...
public class Pretraitement {
	...
	"", "un", "deux", "trois",
	"quatre", "cinq", "sisse", "sept", "huit", "neuf",
    ...
    {
    	int i = 0;
    	boolean estNum= false; // ajout mai 2011
    	...
    	while (i < texte.length() && !estCoupure(tete))
    	{
    		...
    		else if (estOperateur(tete))
    		{
    			if (tete=='+') buffer.append("_plusse_");
    			else if ((tete=='/')&& estNum) buffer.append("_sur_");
    			else if ((tete=='*')&& estNum) buffer.append("_fois_");
    			else if ((tete=='-')&& estNum) buffer.append("_moins_");
    			else if ((tete=='=')&& estNum) buffer.append("_égal_");
    			i++;
    		}
    		...
    		{
    			buffer.append('_');
    			System.out.println("SI_VOX WARNING [Pretraitement.traiter] : le caractere '" 
    					+ tete + "' a ete ignore !");
    			i++;
    		}
    		if (i < texte.length())
    	...
    	if (estCoupure(tete)||(tete=='/'&&!estNum))
    ...
    private String ordinal(String s, Indice i)
    {
    	...
    	if (fin.length() >= 2)
    	{
    		...
    		if (fin.substring(0,3).equals("ième"))
    		{
    			i.inc(3);
    			return "_iaime";
    		}
    		if (fin.substring(0,3).equals("ière"))
    		{
    			i.inc(3);
    			return "_premiaire";
    		}
    ...
    {
    	return c == ',' || c == '.' || c == ';' || c == ':' || c == '!' || c == '?' || estParenthese(c) || c == '|';
    }
