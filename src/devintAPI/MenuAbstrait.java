...
public abstract class MenuAbstrait extends DevintFrameListener implements
	...
	*/
	...
	protected void setPropertiesButton(JButton b) {
        ...
        b.addMouseListener(new MouseOverButton());
	}

    // mettre � jour les propri�t�s des boutons
    protected void refreshButtons() {
        for(JButton b:boutonOption) {
            b.setBackground(foregroundColor);
            b.setForeground(backgroundColor);
            b.setFont(fonteBouton);
            b.setBorder(buttonBorder);
	...
	public void keyPressed(KeyEvent e) {
		...
		if (e.getKeyCode() == KeyEvent.VK_DOWN) {
			if (optionCourante == -1) {
				optionCourante = 0;
				setFocusedButton(optionCourante,true);
			} else {
	...
	public void actionPerformed(ActionEvent ae) {
		changeSource(ae.getSource(), true);
	}

    private void changeSource(Object source, boolean lancer) {
        for (int i = 0; i < nbOption; i++) {
            if (source == boutonOption[i]) {
                if (optionCourante != -1)
                    unFocusedButton(optionCourante);
                optionCourante = i;
                setFocusedButton(optionCourante,lancer);
                if(lancer)
                    lancerOption(i);
            }
        }
	...
	private void setFocusedButton(int i, boolean avecVoix) {
        if(avecVoix) {
            voix.stop();
...
//	}
    ...
    private class MouseOverButton extends MouseAdapter {
        @Override
        public void mouseEntered(MouseEvent e) {
            changeSource(e.getSource(), false);
        }
